...
function love.load()
    ...
    debug = "Press SPACE to sort notes"
end
...
function getStavePosition(pitch)
    ...
    -- Return mapped position or estimate for out-of-range notes
...
function bubbleSortStep()
    ...
    local swapped = false
    for i = 1, #notes - 1 do
        if notes[i].pitch > notes[i + 1].pitch then
            -- Swap notes
            notes[i], notes[i + 1] = notes[i + 1], notes[i]
            
            updatePositions()

            -- Play sound for the note moving **DOWN** in pitch
            playNoteForSorting(notes[i].pitch)

            animationComplete = false
            swapped = true
            debug = string.format("Swapping notes: %d and %d", notes[i].pitch, notes[i + 1].pitch)
            break
        end
    end
